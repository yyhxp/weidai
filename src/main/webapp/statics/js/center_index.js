/**
 * Created by panjunlin on 2016/8/29 0029.
 */
// 左边导航栏创建
mnav.activity(1);
//存管信息
...
$(function () {
    ...
    // 0, "未开通";1, "未激活";2, "正常"
    // 到账户总览页面时，只有从登陆页面跳过来，才会立马展示开通存管通信息，从其他页面跳过来，则不自动展示开通对话框 个人用户
...
// 阻止滚动条滚动 left: 37, up: 38, right: 39, down: 40, spacebar: 32, pageup: 33,
...
var bindSelect = function (_obj) {
    ...
    obj.delegate("dd a", "click", function () {
        ...
        if (this.selected == "selected") {
            ...
            cpli.css({
                zIndex: cpli.attr("data-default-zindex") != undefined
                    ? cpli.attr("data-default-zindex")
                    : ""
            });
            $(this)
                .parents("dd")
    ...
    obj
        ...
        .click(function () {
            ...
            curselect
                .find("dd")
                .css({width: "", height: "", overflowY: "visible"});
            var ul_w = curselect
                .find("ul")
                .width();
            var ul_h = curselect
            ...
            curselect
                .find("dd")
                .css({width: ul_w, height: ul_h, overflowY: overflowY});
            if (ie6) {
...
var datapicker = new DatePicker('datapicker', data, function (year, month, render) {
    var _this = this;
    // $("#selectYear").val(year); $("#selectMonth").val(month);
    ...
    $.ajax({
        type: 'post',
        url: window.action.rootPath + window.action.interface.receivableDetail,
        data: {
            "year": year,
            "month": month
        },
        success: function (res) {
            ...
            if (res.success == true) {
                if (_data.monthRecoverAmount != null && _data.monthRecoverAmount != undefined) {
                    $(".month-data span")
                        .eq(0)
                        .text(formatMoney(_data.monthRecoverAmount));
                } else {
                    $(".month-data span")
                        .eq(0)
                        .text("0");
                ...
                if (_data.monthUnRecoverAmount != null && _data.monthUnRecoverAmount != undefined) {
                    $(".month-data span")
                        .eq(1)
                        .text(formatMoney(_data.monthUnRecoverAmount));
                } else {
                    $(".month-data span")
                        .eq(1)
                        .text("0");
            ...
            dayShouldGet = _data != null
                ? _data.receivable4DayList
                : null;
            dayNoGet = _data != null
                ? _data.receivable4DayList
                : null;
...
//有数据的日期点击后显示对应数据
$(document).on('click', '.weidai-datepicker-day span', function () {
    $("#dataLeft").empty();
    $(this)
        .addClass("today")
        .siblings()
    ...
    if ($(this).hasClass("hasdata")) {
        ...
        var dataLeftHtml = '<h3 class="fonts-gray" style="z-index:3;">应收金额(元)<i class="iconfont icon-tixing">' +
                '&#xe606;</i><br/><span class="fonts-black DinFont">' + formatMoney(dayShouldGet[num].recoverAmount) + '</span><div class="tixing"><i></i>到期应收(元)：' + formatMoney(dayShouldGet[num].expireRecoverAmount) + '<br/>历史提前还款(元)：' + formatMoney(dayShouldGet[num].historyRecoveredAmount) + '(今日到期应回款中借款人已提前还款的部分)<br/><span class="fonts-black">应收金额=到期应收-历史提前还款</span></div>' +
                '</h3><h3  class="fonts-gray" style="z-index:1;">实收金额(元)<i class="iconfont icon-ti' +
                'xing">&#xe606;</i><br/><span class="fonts-black DinFont">' + formatMoney(dayNoGet[num].recoveredAmount) + '</span><div class="tixing"><i></i>到期实收(元)：' + formatMoney(dayShouldGet[num].todayRecoveredAmount) + '<br/>今日提前还款(元)：' + formatMoney(dayShouldGet[num].todayAdvanceAmount) + '(未来应收还款中借款人已提前至今日还款的部分)<br/><span class="fonts-black">实收金额=到期实收+今日提前还款</span></di' +
                'v></h3>';
        ...
        var emptyHtml = '<div class="empty"><img src="' + staticUrl + '/img/center/empty.png" /><p>当天没有项目回款</p><p><a href="/list/showBidList" class="bl' +
                'ueColor">快去投资</a>吧...</p> </div>'
...
//今天对应的数据（应收金额、未收金额）
function todayDate() {
    for (i = 0; i < $(".weidai-datepicker-day span").length; i++) {
        if ($(".weidai-datepicker-day span").eq(i).hasClass("today")) {
            $(".weidai-datepicker-day span")
                .eq(i)
                .trigger("click");
            return false;
        }
...
//年月变动时日历的反应 $('#selectYearTxt').on('change', selectChange);
...
$.ajax({
    type: 'post',
    url: window.action.rootPath + window.action.interface.collectedDetail,
    ...
    success: function (res) {
        ...
        if (res.success == true) {
            var _collectHtml = '';
            var arr = _data.data != null
                ? _data.data.length
                : null;
            // console.log("收款数据："+_data.data); console.log(arr);
            if (arr) {
                for (i = 0; i < arr; i++) {
                    if (_data.data[i] != null && _data.data[i] != undefined) {
                        ...
                        for (var key in _data.data[i]) {
                            ...
                            _collectHtml = "<ul class='content'><li class='tal'>" + _period + "/" + _periods + "</li><li><a href='" + viewUrl + "'>" + formatBidName(_assetName) + "</a></li><li>" + _recoverTime + "</li><li class='tar'>" + formatMoney(_recoverAmount) + "</li><li class='tar'>" + formatMoney(_recoverPrincipal) + "</li><li class='tar'>" + formatMoney(_recoverInterest) + "</li><li>" + _recoverStatus + "</li></ul>";
                        }
...
function depositProcess(url) {
    //存管开关关闭，获取存管开通后，则直接跳转到后台，后台根据用户的存管状态跳转到不通的页面
...
var uid;
uid = $("#hid_uid").val();
noInform = $("#check1").attr("data-value");
$.ajax({
    type: 'post',
    url: window.urlbase + window.action.interface.queryVipWindow,
    data: {
        uid: uid,
        net4: Math.random()
    },
    success: function (res) {
        if (res.resultCode == "1000") {
            if (res.data != null && res.data != "") {
                popLevel = res.data.newLevel;
                popType = res.data.popType;
                if (res.data.popType == "0") {
                    $(".VIPlevel").addClass("VIPlevel" + res.data.newLevel);
                    $(".informWord").html("恭喜您成为微贷网V" + res.data.newLevel + "会员");
                    if (popLevel == 1 || popLevel == 2) {
                        $(".welfare2 ul").html("<li><img  src='" + staticUrl + "/image/welfare1.png' srcset='" + staticUrl + "' alt=''><p>生日献福礼</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare2.png' srcset='" + staticUrl + "/img/center/VIP/welfare2@2x.png 2x' alt=''><p>升级欢迎礼</p> </li><li><img  src='" + staticUrl + "/img/center/VIP/welfare3.png' srcset='" + staticUrl + "/img/center/VIP/welfare3@2x.png 2x' alt=''><p>月度资产分析账单</p></li>")
                    }
                    if (popLevel == 3 || popLevel == 4) {
                        $(".welfare2 ul").html("<li><img  src='" + staticUrl + "/img/center/VIP/welfare1.png' srcset='" + staticUrl + "/img/center/VIP/welfare1@2x.png 2x' alt=''><p>生日献福礼</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare2.png' srcset='" + staticUrl + "/img/center/VIP/welfare2@2x.png 2x' alt=''><p>升级欢迎礼</p> </li><li><img  src='" + staticUrl + "/img/center/VIP/welfare3.png' srcset='" + staticUrl + "/img/center/VIP/welfare3@2x.png 2x' alt=''><p>月度资产分析账单</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare4.png' srcset='" + staticUrl + "/img/center/VIP/welfare4@2x.png 2x' alt=''> <p>固定收益特权</p></li>")
                    }
                    if (popLevel == 5 || popLevel == 6) {
                        $(".welfare2 ul").html("<li><img  src='" + staticUrl + "/img/center/VIP/welfare1.png' srcset='" + staticUrl + "/img/center/VIP/welfare1@2x.png 2x' alt=''><p>生日献福礼</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare2.png' srcset='" + staticUrl + "/img/center/VIP/welfare2@2x.png 2x' alt=''><p>升级欢迎礼</p> </li><li><img  src='" + staticUrl + "/img/center/VIP/welfare3.png' srcset='" + staticUrl + "/img/center/VIP/welfare3@2x.png 2x' alt=''><p>月度资产分析账单</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare4.png' srcset='" + staticUrl + "/img/center/VIP/welfare4@2x.png 2x' alt=''> <p>固定收益特权</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare5.png' srcset='" + staticUrl + "/img/center/VIP/welfare5@2x.png 2x' alt=''><p>新产品上线优先测试</p> </li>")
                    }
                    $(".seeInfo").html("查看会员福利");
                    $(".seeInfo").after("<p><input type='checkbox' id='check1' data-value='0'/>再次升级至该等级时不再提示</p> ");
                    $(".welfare2").show();
                    $("#VIPinform").show();
                }
                if (res.data.popType == "1") {
                    $(".VIPlevel").addClass("VIPlevel" + res.data.newLevel);
                    $(".informWord").html("您的会员等级已下降至V" + res.data.newLevel + "会员");
                    if (popLevel == 1 || popLevel == 2) {
                        $(".welfare2 ul").html("<li><img  src='" + staticUrl + "/img/center/VIP/welfare1.png' srcset='" + staticUrl + "/img/center/VIP/welfare1@2x.png 2x' alt=''><p>生日献福礼</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare2.png' srcset='" + staticUrl + "/img/center/VIP/welfare2@2x.png 2x' alt=''><p>升级欢迎礼</p> </li><li><img  src='" + staticUrl + "/img/center/VIP/welfare3.png' srcset='" + staticUrl + "/img/center/VIP/welfare3@2x.png 2x' alt=''><p>月度资产分析账单</p></li>")
                    }
                    if (popLevel == 3 || popLevel == 4) {
                        $(".welfare2 ul").html("<li><img  src='" + staticUrl + "/img/center/VIP/welfare1.png' srcset='" + staticUrl + "/img/center/VIP/welfare1@2x.png 2x' alt=''><p>生日献福礼</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare2.png' srcset='" + staticUrl + "/img/center/VIP/welfare2@2x.png 2x' alt=''><p>升级欢迎礼</p> </li><li><img  src='" + staticUrl + "/img/center/VIP/welfare3.png' srcset='" + staticUrl + "/img/center/VIP/welfare3@2x.png 2x' alt=''><p>月度资产分析账单</p></li><li><img  src='" + staticUrl + "/img/center/VIP/welfare4.png' srcset='" + staticUrl + "/img/center/VIP/welfare4@2x.png 2x' alt=''> <p>固定收益特权</p></li>")
                    }
                ...
                if (res.data.popType == "2") {
                    ...
                    $(".VIPword")
                        ...
                        .show();
                    $(".goSee").show();
                    $(".seeInfo").html("查看我的升级欢迎奖励");
                    $(".seeInfo").addClass("goMyPacket");
                    $("#VIPinform").show();
                }
                if (res.data.popType == "3") {
                    $(".VIPlevel").addClass("VIPlevelBoss");
                    $(".informWord").html("恭喜您升级为微贷会员俱乐部会员");
                    $(".VIPword")
                        .html("特送上开启收益特权奖励！")
                        .show();
                    $(".goSee").show();
                    $(".seeInfo").html("查看我的收益特权奖励");
                    // $(".seeInfo").addClass("goMyPacket");
                    $("#VIPinform").show();
                }
            }
...
$(document).on("click", ".seeInfo", function () {
    if (checkVal) {
        $.ajax({
            type: 'post',
            url: window.urlbase + window.action.interface.neverShowWindow,
            data: {
                uid: uid,
                level: popLevel,
                popType: popType,
    ...
    window.location.href = "https://www.weidai.com.cn/vip/index.html";
})
//前往我的红包
$(document).on("click", ".goMyPacket", function () {
    window.location.href = "https://www.weidai.com.cn/account/addRateRedPacket";
})

var allHeight;

function getHeight() {
    allHeight = $(".VIPalert").height();
}
var int = self.setInterval("getHeight()", 100);
setTimeout(function () {
    window.clearInterval(int);
    var itHeight = ($(window).height() - allHeight) / 2;
    $(".VIPalert").css("top", itHeight);
}, 1000)

//等级变动不在提示
var checkVal;
$(document).on("click", "#check1", function () {
    checkVal = $("#check1").attr("data-value");
    if (checkVal == "0") {
        checkVal = 1;
    } else {
        checkVal = 0;
...
//点击页面其他地方关闭弹框
$("#VIPinform").click(function (e) {
if ($(e.target).closest(".VIPalert").length) {
        return;
    }
    $('#VIPinform').fadeOut();
})
...
//查询账户中心热售标的 doT模板引入
function news(dotId, htmlId) {
    $.ajax({
        type: 'get',
        url:"/newIndexRecommendBid",
        success: function (res) {
            if (res.success == true) {
                var _data = res.data;
                var interText = doT.template($(dotId).html());
                $(htmlId).html(interText(_data));
...
news("#accountRecommendBidBox", "#accountRecommendBid");