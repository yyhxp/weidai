...
function CenterNav(setting) {
	...
	this.init = function() {
		for (var i = 0; i < this.name.length; i++) {
			$("#mNavId ul").append("<li><i class='iconfont'>" + this.icon[i] + "</i>" + this.name[i] + "</li>");
		}
	};
...
$.ajax({
	type: 'get',
	url: window.action.rootPath + window.action.interface.useableCount,
	success: function(result) {
		if (result.success) {
			//var unused = result.data.UNUSED;
			var unused = result.data;
			if(unused == null || unused == undefined){
				unused = 0;
			}
			var redHtml = '<i class="tips">'+unused+'</i>';
			if(unused > 0){
				$("#mNavId li").eq(8).append(redHtml);
			}
...
function changeStatus(status,classname,tipinfo,url) {
	var tipHtml1 = "<div class='tip_info tl'>您还未"+tipinfo+"<a href="+url+">管理</a></div>";
	var tipHtml2 = "<div class='tip_info tl'>您已"+tipinfo+"<a href="+url+">管理</a></div>";
	if(status==0){
...
$.ajax({
	type: 'get',
	url: window.action.rootPath + window.action.interface.userInfo,
	success: function (res) {
		...
		if(_accountType == 1){
			var tipHtml1 = "<div class='tip_info tl'>您还未开通存管账户<a href="+cunguanUrl+">开通</a></div>";
			var tipHtml2 = "<div class='tip_info tl'>您已开通存管账户<a href="+cunguanUrl+">查看</a></div>";
...
function getLevel(levelNum) {
    var levelNum;
    $(".level").addClass("level" + levelNum);
}
setTimeout(function () {
    var curLevel = $("#hid_level").val();
    getLevel(curLevel);
}, 200)
